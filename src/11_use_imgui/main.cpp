...
#include <tool/gui.h> // imgui resource
using namespace std;
...
int main()
{
	...
	// 绘制物体的着色器
	Shader ourShader("./src/11_use_imgui/vs.glsl", "./src/11_use_imgui/fs.glsl");

	// 绘制光源的着色器
	Shader lampShader("./src/11_use_imgui/lightCube_vs.glsl", "./src/11_use_imgui/lightCube_fs.glsl");
	...
	float vertices[] = {
			...
			0.5f, 0.5f, 0.5f, 0.0f, 1.0f, 0.0f,
			-0.5f, 0.5f, 0.5f, 0.0f, 1.0f, 0.0f,
			-0.5f, 0.5f, -0.5f, 0.0f, 1.0f, 0.0f}; // 顶部
	...
	unsigned char *data = stbi_load("./static/texture/container.jpg", &width, &height, &nChannels, 0);
	if (data)
	{
	...
	//unsigned char *data = stbi_load("C:/Users/ithan/Desktop/container.jpg", &width, &height, &nChannels, 0);
	data = stbi_load("./static/texture/awesomeface.png", &width, &height, &nChannels, 0);
	if (data)
	{
